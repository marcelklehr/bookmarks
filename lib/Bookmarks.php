...
class Bookmarks {
	...
	public function hashFolder($userId, $folderId, $fields) {
		...
		$childHashes = array_map(function ($item) use ($userId, $fields) {
			switch ($item['type']) {
				...
				case 'folder':
				  return $this->hashFolder($userId, $item['id']);
			  default:
				  throw new UnexpectedValueException('Expected bookmark or folder, but not '.$item['type']);
			}
		}, $children);
		$folder = ['id' => $folderRecord['id'], 'title' => $folderRecord['title'], 'children' => $childHashes];
	...
	public function hashBookmark($userId, $bookmarkId, $fields) {
		...
		return Marmur2Hash::hash(json_encode($bookmark));
	}
